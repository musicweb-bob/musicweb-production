...
export function Reviews({ onNavigate: _onNavigate }: ReviewsProps) {
  ...
  // --- 1. THE "MASTERPIECE" SAFETY NET ---
  const BACKUP_REVIEWS: ReviewItem[] = [
    { id: 'b1', artist: "Radiohead", album: "OK Computer", score: "10.0", verdict: "MASTERPIECE", summary: "The definitive reissue of the album that predicted the 21st century.", color: "from-blue-600 to-sky-900", link: "#", source: "Classic", thumbnail: "https://images.unsplash.com/photo-1614613535308-eb5fbd3d2c17?auto=format&fit=crop&q=80&w=800" },
    { id: 'b2', artist: "Kendrick Lamar", album: "Mr. Morale", score: "8.9", verdict: "BEST NEW MUSIC", summary: "A dense, thorny, and deeply personal double album.", color: "from-amber-700 to-orange-900", link: "#", source: "Classic", thumbnail: "https://images.unsplash.com/photo-1511192336575-5a79af67a629?auto=format&fit=crop&q=80&w=800" },
    { id: 'b3', artist: "Daft Punk", album: "R.A.M.", score: "9.2", verdict: "ESSENTIAL", summary: "A dazzling, meticulously crafted love letter to disco and funk.", color: "from-purple-600 to-indigo-900", link: "#", source: "Classic", thumbnail: "https://images.unsplash.com/photo-1493225255756-d9584f8606e9?auto=format&fit=crop&q=80&w=800" }
  ];

  // --- 2. LIVE AGGREGATION ENGINE (Pitchfork + NME) ---
  const fetchReviews = async () => {
    ...
    try {
      const promises = FEEDS.map(async (feed) => {
        ...
        } catch (e) { console.warn(`Feed ${feed.name} failed.`); }
      });
  ...
  return (
    <div className="min-h-screen bg-black text-white pt-6 pb-24 px-8 font-sans relative overflow-x-hidden">
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12 flex flex-col items-center border-b border-white/10 pb-10">
          ...
          <div className="flex items-center gap-4 mt-6">
            <button onClick={fetchReviews} className="flex items-center gap-2 bg-zinc-900/50 hover:bg-white hover:text-black border border-white/5 px-5 py-2.5 rounded-full transition-all group backdrop-blur-md">
            ...
            {reviews.map((r) => (
              <div key={r.id} className="group bg-zinc-900/40 border border-white/5 rounded-[2.5rem] overflow-hidden hover:-translate-y-2 transition-all relative backdrop-blur-sm hover:border-orange-500/30 shadow-2xl">
                ...
                <div className="h-48 relative overflow-hidden bg-black">
                  <img src={r.thumbnail} alt="" className="w-full h-full object-cover opacity-60 group-hover:opacity-100 group-hover:scale-110 transition-all duration-1000" />
